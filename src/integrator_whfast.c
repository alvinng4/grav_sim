...
IN_FILE void whfast_compute_eta(double *restrict eta, const System *restrict system);
...
)
{
    ...
    *num_steps_ptr = 0;
    while (*num_steps_ptr < total_num_steps)
    {
        ...
        /* Sort by distance */
        // TODO: Modify this with insertion sort since it is faster for mostly sorted
        // array
        x = system->x;
        v = system->v;
        system->x = jacobi_x;
...
#pragma omp parallel for
    ...
    for (int i = 1; i < num_particles; i++)
    {
        ...
        for (int j = 0; j < WHFAST_KEPLER_MAX_ITER; j++)
        {
            ...
            const double F =
                (x_norm * s * c1 + x_norm * radial_v * (s * s) * c2 +
                 gm * (s * s * s) * c3 - dt);
            const double dF =
                (x_norm * c0 + x_norm * radial_v * s * c1 + gm * (s * s) * c2);
        ...
        if (local_error_status.return_code == GRAV_SUCCESS)
        {
            ...
            if (!is_converged)
            {
                ...
                if (verbose >= GRAV_VERBOSITY_VERBOSE)
                {
                    fprintf(
                        stderr,
                        "%s: Kepler's equation did not converge for particle id %d. "
                        "Error: %23.15g\n",
                        __func__,
                        particle_ids[i],
                        error
                ...
                if ((error > WHFAST_INVALID_TOL || is_z_not_finite) &&
                    remove_invalid_particles)
                {
         ...
         z / 20.0 *
             (1.0 -
              z / 42.0 *
                  (1.0 -
                   z / 72.0 *
                       (1.0 - z / 110.0 * (1.0 - z / 156.0 * (1.0 - z / 210.0)))))) /
        6.0;
    double temp_c2 =
        (1.0 -
         z / 12.0 *
             (1.0 -
              z / 30.0 *
                  (1.0 -
                   z / 56.0 *
                       (1.0 - z / 90.0 * (1.0 - z / 132.0 * (1.0 - z / 182.0)))))) /
        2.0;
...
)
{
    ...
    const double softening_length_cube =
        softening_length * softening_length * softening_length;
...
#pragma omp parallel for
    ...
    for (int i = 1; i < num_particles; i++)
    {
        ...
        temp_vec_norm_cube =
            (temp_vec_norm * temp_vec_norm * temp_vec_norm) + softening_length_cube;
        temp_jacobi_norm = vec_norm_3d(&jacobi_x[i * 3]);
        temp_jacobi_norm_cube =
            (temp_jacobi_norm * temp_jacobi_norm * temp_jacobi_norm) +
            softening_length_cube;
        for (int j = 0; j < 3; j++)
        {
