...
#pragma omp parallel for
    ...
    for (int i = 0; i < num_particles; i++)
    {
        const double x_normalized =
            (x[i * 3 + 0] - box_center[0] + box_width) * inv_cell_length;
        const double y_normalized =
            (x[i * 3 + 1] - box_center[1] + box_width) * inv_cell_length;
        const double z_normalized =
            (x[i * 3 + 2] - box_center[2] + box_width) * inv_cell_length;
        ...
        delta
            [(n_x % pm_grid_size) * grid_size_2 + (n_y % pm_grid_size) * pm_grid_size +
             (n_z % pm_grid_size)] += m[i] * weight_x_m * weight_y_m * weight_z_m;
        delta
            [((n_x + 1) % pm_grid_size) * grid_size_2 +
             (n_y % pm_grid_size) * pm_grid_size + (n_z % pm_grid_size)] +=
            m[i] * weight_x * weight_y_m * weight_z_m;
        delta
            [(n_x % pm_grid_size) * grid_size_2 +
             ((n_y + 1) % pm_grid_size) * pm_grid_size + (n_z % pm_grid_size)] +=
            m[i] * weight_x_m * weight_y * weight_z_m;
        delta
            [(n_x % pm_grid_size) * grid_size_2 + (n_y % pm_grid_size) * pm_grid_size +
             ((n_z + 1) % pm_grid_size)] += m[i] * weight_x_m * weight_y_m * weight_z;
        delta
            [((n_x + 1) % pm_grid_size) * grid_size_2 +
             ((n_y + 1) % pm_grid_size) * pm_grid_size + (n_z % pm_grid_size)] +=
        ...
        delta
            [((n_x + 1) % pm_grid_size) * grid_size_2 +
             (n_y % pm_grid_size) * pm_grid_size + ((n_z + 1) % pm_grid_size)] +=
            m[i] * weight_x * weight_y_m * weight_z;
        delta
            [(n_x % pm_grid_size) * grid_size_2 +
             ((n_y + 1) % pm_grid_size) * pm_grid_size + ((n_z + 1) % pm_grid_size)] +=
    ...
    for (int i = 0; i < pm_grid_size; i++)
    {
        ...
        for (int j = 0; j < pm_grid_size; j++)
        {
            ...
            for (int k = 0; k < pm_grid_size; k++)
            {
                ...
                acc_grid[index * 3 + 0] = (phi[index_x_m_2] - 8.0 * phi[index_x_m] +
                                           8.0 * phi[index_x_p] - phi[index_x_p_2]) /
                                          (12.0 * cell_length);
                acc_grid[index * 3 + 1] = (phi[index_y_m_2] - 8.0 * phi[index_y_m] +
                                           8.0 * phi[index_y_p] - phi[index_y_p_2]) /
                                          (12.0 * cell_length);
                acc_grid[index * 3 + 2] = (phi[index_z_m_2] - 8.0 * phi[index_z_m] +
                                           8.0 * phi[index_z_p] - phi[index_z_p_2]) /
                                          (12.0 * cell_length);
            }
        }
    }
...
)
{
    ...
    fftw_complex *delta_fourier =
        fftw_malloc(grid_size_2 * (pm_grid_size / 2 + 1) * sizeof(fftw_complex));
    ...
    fftw_plan plan_forward = fftw_plan_dft_r2c_3d(
        pm_grid_size, pm_grid_size, pm_grid_size, delta, delta_fourier, FFTW_ESTIMATE
    );
    if (!plan_forward)
    {
        error_status = WRAP_RAISE_ERROR(
    ...
    const double two_pi_over_box_length_squared =
        4.0 * M_PI * M_PI / (box_length * box_length);
#ifdef USE_OPENMP
#pragma omp parallel for
#endif
    for (int i = 0; i < pm_grid_size; i++)
    {
        ...
        for (int j = 0; j < pm_grid_size; j++)
        {
            ...
            {
                const int k_z = k;
                const double k_sq = (k_x * k_x + k_y * k_y + k_z * k_z) *
                                    two_pi_over_box_length_squared;

                const double kernel = k_sq == 0.0 ? 0.0 : -1.0 / k_sq;

                const int index =
                    i * (pm_grid_size * half_grid_size) + j * half_grid_size + k;
    ...
    fftw_plan plan_backward = fftw_plan_dft_c2r_3d(
        pm_grid_size, pm_grid_size, pm_grid_size, delta_fourier, phi, FFTW_ESTIMATE
    );
    if (!plan_backward)
    {
        error_status = WRAP_RAISE_ERROR(
